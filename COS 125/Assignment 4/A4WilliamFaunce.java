// William Faunce
// This program reads data from a txt file then reconfigures and outputs to a new file
...
public class A4WilliamFaunce {
	...
	public static void main(String[] args) throws IOException {
		...
		writer.println(String.format("%-12s %-12s %-12s %-12s %-12s %-12s %-12s", "Town", "Date", "Temp", "Direction", "Wind Speed", "Wind Chill", "Reporter"));
		
		// scans and assigns inputs while the weather.txt has nonempty data sets
		while (filescan.hasNext()) {
			town = filescan.next();
			town = town.toUpperCase();													// capitalizes the town/city name
			date = filescan.next();
			...
			double windC = (35.74 + (0.6215 * temp) - (35.75 * (Math.pow(windS, 0.16))) + (0.4275 * temp * (Math.pow(windS, 0.16))));
		...
		writer.println("");
		writer.println("Average Temperature: " +fmt.format(avgTemp));
		...
		writer.println(String.format("%-10s %-5d", "CALAIS", cCount));
		writer.println("");
		...
		writer.println("");
		writer.println("Difference between this " +month+ " from historical average is " +fmt.format(diff));
